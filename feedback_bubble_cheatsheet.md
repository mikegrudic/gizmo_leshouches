...
| **SNe: free expansion** | $R_s \approx v_{\rm ej}; t$ | $R_s \propto t$ | Mass swept out is $<<M_{\rm ej}$ |
| **SNe: Sedov-Taylor** | $R_s = \left(\dfrac{2.026; E}{\rho_0}\right)^{\!1/5}\! t^{2/5}$ | $R_s \propto t^{2/5}$ | Mass swept out is $>>M_{\rm ej}$ |
...
| $c_s$ | Sound speed in the ionized gas, $c_s = \sqrt{2k_BT/m_H} \approx 13;{\rm km;s^{-1}}$ at $T=10^4;{\rm K}$ |
| $\mathcal{Q}$ | Ionizing photon luminosity (${\rm photons;s^{-1}}$) |
| $\alpha_B$ | Case B recombination coefficient, $\alpha_B \approx 2.54\times10^{-13} T_4^{-0.833};{\rm cm^3;s^{-1}}$, where $T_4 = T/10^4;{\rm K}$ |
...
| $E$ | Supernova explosion energy ($\approx 10^{51};{\rm erg}$) |
